use tauri::command;
use std;
use crate::commands::images::{ filter_aliasing, merge_multiple_images, from_rgb_to_binary, from_multiples_masks_to_multiclass };
use std::fs::File;
    ...
    Ok(re) => {
      for path in paths {
        ...
        if path.is_dir() && recursive {
          files.append(
            list_files_in_folder(path.to_str().unwrap(), regexfilter, recursive).as_mut()
          );
        }
      }
    }
...
pub fn load_xml_file(filepath: String) -> Result<String, String> {
  ...
  file.read_to_string(&mut xml_content).map_err(|e| format!("Failed to read file: {}", e))?;
...
pub fn load_json_file(filepath: String) -> Result<String, String> {
  ...
  file.read_to_string(&mut json_content).map_err(|e| format!("Failed to read file: {}", e))?;
...
fn read_mask_and_save(
  ...
  images.iter().for_each(|image| {
    ...
    let image_data = general_purpose::STANDARD.decode(image_data.as_bytes()).unwrap();
  ...
  if combined_mask || individual_mask {
    let binary_masks: Vec<ImageBuffer<image::Luma<u8>, Vec<u8>>> = masks
      .iter()
      .map(|mask| from_rgb_to_binary(mask))
      .collect();
